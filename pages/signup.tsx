...
interface PageContext {
    signup_obj: SignUp,
    current_step: number,
    set_error_message: (error: boolean, message: string, element: HTMLParagraphElement) => void,
...
export default function Sign_up_page() {
    ...
    function set_error_message(error: boolean, message: string, error_element: HTMLParagraphElement) {
            ...
            try {
                ...
                timer = setTimeout(async() => {
                    ...
                    if(username.length === 0) {
                        ...
                        set_error_message(true, "Please enter a username.", error_element)
                        resolve(false)

                    } else if(username.length < 3) {

                        set_error_message(true, "Username is to short. Min. 3 characters.", error_element)
                        resolve(false)

                    } else if(username.length > 16) {

                        set_error_message(true, "Username is to long. Max. 16 characters.", error_element)
                        resolve(false)
                        ...
                        if(beginning_regex.test(username) === true || end_regex.test(username) === true) {
                            ...
                            set_error_message(true, "You can't use special characters at the beginning or end of your username.", error_element)
                            resolve(false)
                            ...
                            if(look_double_special_characters_regex.test(username)) {
                                ...
                                set_error_message(true, "Username cannot contain 2 special characters after each other.", error_element)
                                resolve(false)
                                ...
                                if(username_regex.test(username) === true){
                                    ...
                                    const response = await fetch(`/api/signup/validate_username?username=${username}`)
                                    const username_available = (await response.json()).available
        
                                    //If username is available update signup_obj state.
                                    if(username_available) {
                                        set_error_message(false, "", error_element)
                                        resolve(true)
                                        
        
                                    } else {

                                        set_error_message(true, "Username is already taken.", error_element)
                                        resolve(false)
                                    ...
                                    set_error_message(true, "Special characters that are allowed: . _", error_element)
                                    resolve(false)
            ...
            try {
                ...
                timer = setTimeout(() => {
                    if(email_regex.test(email) === true) {
                        ...
                        set_error_message(true, "Please enter a valid Email.", error_element)
                        resolve(false)
                    }
                }, 200);
...
export function Step_3() {
    ...
    return (
        <motion.div className="step_container" initial={{opacity: 0, y: -50}} animate={{opacity: 1, y: 0, transition: {duration: .3, delay: 0.2}}} exit={{opacity: 0, y: 50, transition: {duration: 0.2,  type: "tween"}}}>
            ...
            <input  onBlur={get_input_value} onKeyUp={get_input_value} type="password" placeholder={"Password"} id="input" />
            <p className="input_error_message" id="input_error_message_password"></p>
            <input onBlur={get_input_value} onKeyUp={get_input_value} type="password" placeholder={"Password-repeat"} id="input" />
            <p className="input_error_message" id="input_error_message_password_repeat"></p>
            
            <div className="legal_container">
                <div className="check_box_container">

                    <div onClick={() => PAGE_CONTEXT.update_signup_informations("legal", !PAGE_CONTEXT.signup_obj.legal)} className="check_box">
            ...
            <div className="button_container">
                <button className="disabled_button" id="step_button" >Create Account</button>
            </div>
