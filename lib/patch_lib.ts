...
export class PatchHandler {
    ...
    constructor(patchDirectory: string = directoryOfPatches) {
        ...
        function createOrderedPatchnoteList(patchnoteList: Patchnote[]): Patchnote[] | [] {
            if(patchnoteList.length === 0) return []
    ...
    getPatchnote(id: string): Patchnote | null {
    ...
    constructor(patchDirectory: string ,id: string) {
        ...
        function getInfo(directory: string, id: string): Patchnote_info {
            try {
                ...
                const mdPatchInfo =  matterResult.data as { title?: string, update?: string, image?: string, author?: string, date?: string}
                ...
                function patchInfoDateValidator(date:string | undefined) {
                    if(!date) return new Date(1990, 1, 1)
                    const formatted_date = date
                    if(formatted_date.split("/").length !== 3) new Date(1990, 1, 1)

                    const day = parseInt(formatted_date.split("/")[0])
                    const month = parseInt(formatted_date.split("/")[1]) - 1
                    const year = parseInt(formatted_date.split("/")[2])
